...
function AppContent() {
  ...
  const handleSaveDraft = async (formData) => {
    try {
      ...
      if (registeredMobile) {
        await AsyncStorage.setItem(`@draft_form_${registeredMobile}`, JSON.stringify(formData));
      }
    } catch (e) {
  ...
  const handleViewDraftsFromNav = () => {
    if (!registeredMobile) {
      Alert.alert("Notice", "Please login with your mobile number first to view drafts.");
      return;
    }
  ...
  const handleSubmitFinal = async (finalData) => {
    setSubmittedApplications((prev) => [...prev, { ...finalData, mobile: registeredMobile, submittedAt: new Date().toLocaleDateString() }]);
...
function ActionChoiceScreen({ mobileNumber, savedDraft, onFillNewApp, resumeDraft, onViewStatus, onBackToOtp }) {
  return (
      ...
      <View style={styles.adminContainer}>
        ...
        <View style={styles.adminHeader}>
          ...
          <Text style={styles.adminSubtitle}>
            Verified Mobile: +91 {mobileNumber || "9876543210"}. Choose an action below to proceed.
          </Text>
        </View>
        ...
        <View style={styles.portalCardsList}>
          {savedDraft ? (
            <View style={styles.draftContainerBox}>
              <Text style={styles.sectionHeaderTitle}>📂 Saved Draft Application</Text>
              <TouchableOpacity
              ...
              >
                <View style={styles.draftTextGroup}>
                  <Text style={styles.draftItemTitle}>
                    {savedDraft.memberName ? `Name: ${savedDraft.memberName}` : "Incomplete Application Form"}
                  </Text>
                  <Text style={styles.draftItemSub}>
                    Enterprise: {savedDraft.enterpriseName || "Not Specified"} | Step {savedDraft.activeFormStep || 1} of 4
                  </Text>
                </View>
          ...
          >
            ...
            <View style={styles.portalCardTextGroup}>
              <Text style={styles.portalCardTitle}>Fill New Application Form</Text>
              <Text style={styles.portalCardDesc}>Start a fresh Mahila Credit Card application pipeline</Text>
            </View>
          ...
          >
            <Text style={styles.portalCardIcon}>📊</Text>
            <View style={styles.portalCardTextGroup}>
              <Text style={styles.portalCardTitle}>View Application Status</Text>
              <Text style={styles.portalCardDesc}>Check verification pipeline status of your submitted forms</Text>
            </View>
            <Text style={styles.portalCardArrow}>→</Text>
          </TouchableOpacity>
        </View>
            ...
            {userSubmissions.map((sub, index) => (
              <View key={index} style={styles.statusCard}>
                <View style={styles.statusRowTop}>
                  <Text style={styles.statusAppId}>Application #{1001 + index}</Text>
                  <View style={styles.statusBadge}><Text style={styles.statusBadgeText}>Under BMM Review</Text></View>
                </View>
                <Text style={styles.statusDetail}>👤 Name: {sub.fullName || "Beneficiary"}</Text>
                <Text style={styles.statusDetail}>📍 District: {sub.district} | Block: {sub.block}</Text>
                <Text style={styles.statusDetail}>💳 Requested Limit: ₹{sub.amountRequested || "N/A"}</Text>
                <Text style={styles.statusDate}>Submitted on: {sub.submittedAt}</Text>
              </View>
            ))}
          </ScrollView>
        ) : (
...
const styles = StyleSheet.create({
  ...
  portalCard: { flexDirection: "row", alignItems: "center", backgroundColor: "#ffffff", padding: 16, borderRadius: 16, borderWidth: 1, borderColor: "#e2e8f0", shadowColor: "#000", shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.05, shadowRadius: 2, elevation: 2, gap: 14 },
  draftContainerBox: { backgroundColor: "#fef3c7", borderColor: "#fcd34d", borderWidth: 1, padding: 12, borderRadius: 16, gap: 8 },
  sectionHeaderTitle: { fontSize: 12, fontWeight: "700", color: "#78350f", textTransform: "uppercase" },
  ...
  placeholderContainer: { flex: 1, justifyContent: "center", alignItems: "center", padding: 24, gap: 8 },
  ...
  statusCard: { backgroundColor: "#ffffff", padding: 16, borderRadius: 16, borderWidth: 1, borderColor: "#cbd5e1", gap: 6 },
  statusRowTop: { flexDirection: "row", justifyContent: "space-between", alignItems: "center", marginBottom: 4 },
  statusAppId: { fontSize: 13, fontWeight: "800", color: "#0f172a" },
  statusBadge: { backgroundColor: "#fef3c7", borderColor: "#fcd34d", borderWidth: 1, paddingHorizontal: 8, paddingVertical: 2, borderRadius: 6 },
  statusBadgeText: { fontSize: 10, fontWeight: "700", color: "#b45309" },
  statusDetail: { fontSize: 12, color: "#334155" },
  statusDate: { fontSize: 10, color: "#94a3b8", marginTop: 4, fontFamily: Platform.OS === "ios" ? "Courier" : "monospace" },
});